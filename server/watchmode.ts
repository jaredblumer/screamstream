...
class WatchmodeAPI {
  ...
  async searchTitles(params: {
    ...
    source_ids?: number[];
    type?: "movie" | "tv";
    sort_by?: "popularity_desc" | "popularity_asc" | "release_date_desc" | "release_date_asc" | "relevance_desc" | "relevance_asc" | "title_desc" | "title_asc";
    page?: number;
    limit?: number;
    ...
    return this.makeRequest<{ titles: WatchmodeSearchResult[]; total_results: number; total_pages: number }>('/list-titles/', searchParams);
  }
  ...
  async getTitleSources(titleId: number): Promise<Array<{
    source_id: number;
    name: string;
    type: string;
    region: string;
  ...
  async searchByImdbId(imdbId: string): Promise<WatchmodeSearchResult[]> {
    ...
    const results = await this.makeRequest<{ title_results: WatchmodeSearchResult[] }>('/autocomplete-search/', params);
  ...
  async getRecentReleases(params: {
    source_ids?: number[];
    ...
    const result = await this.makeRequest<{ releases: WatchmodeRelease[] }>('/releases/', searchParams);
...
export async function convertWatchmodeTitleToContent(
      ...
      if (!tvdbPosterUrl) {
        try {
          ...
          const match = searchResults.find(result => {
            ...
            console.log(`  Checking: "${result.name}" (${result.year}) - Year match: ${yearMatches}, Title match: ${titleMatches}`);
          ...
          if (match?.image_url) {
            ...
            console.log(`TVDB poster found for "${title.title}" -> "${match.name}" (${match.year})`);
          } else {
  ...
  const finalPosterUrl = tvdbPosterUrl || getOptimalPosterUrl(title.poster || '', 'medium') || '/posters/default_poster.svg';
 ...
 */
export function getOptimalPosterUrl(posterUrl: string, size: 'small' | 'medium' | 'large' = 'medium'): string {
  if (!posterUrl) return '';
 ...
 */
export function getOptimalBackdropUrl(backdropUrl: string, size: 'small' | 'medium' | 'large' = 'large'): string {
