...
export function registerAdminContentRoutes(app: Express) {
  app.get("/api/admin/content", requireAdmin, async (_req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.post("/api/admin/content", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to create content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.patch("/api/admin/content/:id", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.delete("/api/admin/content/:id", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to delete content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.get("/api/admin/content/hidden", requireAuth, requireAdmin, async (_req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch hidden content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.post("/api/admin/content/:id/hide", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to hide content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.post("/api/admin/content/:id/show", requireAuth, requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to show content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
}
