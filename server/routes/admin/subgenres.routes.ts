...
export function registerAdminSubgenreRoutes(app: Express) {
  ...
  app.get("/api/admin/subgenres/:id", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch subgenre", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.post("/api/admin/subgenres", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(400).json({ message: "Failed to create subgenre", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.patch("/api/admin/subgenres/:id", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to update subgenre", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.delete("/api/admin/subgenres/:id", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to delete subgenre", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.put("/api/admin/subgenres/reorder", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to reorder subgenres", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
}
