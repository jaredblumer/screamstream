...
export function registerPlatformImageRoutes(app: Express) {
  ...
  app.post("/api/admin/platform-images", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(400).json({ message: "Failed to create platform image", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
  ...
  app.put("/api/admin/platform-images/:id", requireAdmin, async (req, res) => {
    try {
      ...
      res.status(400).json({ message: "Failed to update platform image", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
