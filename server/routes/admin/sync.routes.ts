...
export async function registerAdminSyncRoutes(app: Express) {
  app.post('/api/admin/sync-new-to-streaming', requireAdmin, async (req, res) => {
    try {
      ...
      const recentWithImdb = releases
        .filter((r) => r.imdb_id && r.source_name !== 'Shudder')
        .slice(0, 5);
      ...
      for (const release of recentWithImdb) {
        try {
          ...
          if (release.type === 'tv_series') {
            ...
            hasHorrorGenre = tvdbSeries?.genres?.some((g) =>
            ...
            hasHorrorGenre = tvdbMovie?.genres?.some((g) =>
              ['Horror', 'Thriller', 'Mystery', 'Suspense'].includes(g.name)
            );
            if (hasHorrorGenre) horrorReleases.push(release);
          }
      ...
      const newContent = horrorReleases.slice(0, 15).map((release, index) => {
        ...
        return {
          ...
          description:
            ...
            `A ${type} available on streaming platforms${release.genre_names ? ` - Genres: ${release.genre_names.join(', ')}` : ''}`,
        ...
        try {
          ...
          if (!isDuplicate) {
            ...
            const existing = existingContent.find(
              (e) =>
                e.title.toLowerCase() === content.title.toLowerCase() && e.year === content.year
            );
      ...
      console.log(
        `SYNC COMPLETE: Added ${summary.newTitlesAdded} new titles, skipped ${summary.duplicatesSkipped} duplicates`
      );
      res.json(summary);
    } catch (error) {
  ...
  app.post('/api/admin/sync-posters', requireAdmin, async (_req, res) => {
    try {
      ...
      const itemsMissingPosters = existingContent.filter(
        (item) => !item.posterUrl || item.posterUrl === defaultPosterUrl
      );
