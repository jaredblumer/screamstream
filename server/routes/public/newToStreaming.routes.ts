  ...
  app.get("/api/new-to-streaming", async (req, res) => {
    try {
      ...
      const recentReleases = allContent
        .filter(content => 
          content.sourceReleaseDate && 
          content.sourceReleaseDate >= thirtyDaysAgo
        )
        .sort((a, b) => new Date(b.sourceReleaseDate!).getTime() - new Date(a.sourceReleaseDate!).getTime())
        .slice(0, 20);
      ...
      res.status(500).json({ message: "Failed to fetch new streaming content", error: error instanceof Error ? error.message : "Unknown error" });
    }
  });
}
